...
class Main extends React.Component {
  ...
  constructor(props) {
    ...
    this.socket = SocketIOClient('http://192.168.1.64:3000');
  ...
  componentDidMount() {
    ...
    this.setState({userId: decoded.Usuario}, () => {
      axios.get(`http://192.168.1.64:3000/get-10?index=${this.state.index}`)
  ...
  onSend(messages=[]) {
    axios.post(`http://192.168.1.64:3000/post-message`, {message: messages[0], userInput: {Usuario: this.state.userId}}, {headers: {authorization: this.props.navigation.getParam("token")}})
